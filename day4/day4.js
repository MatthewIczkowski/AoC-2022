...
fs.readFile('day4/input.txt', (err, data) => {
    ...
    for(var i = 0; i < lines.length; i++){
        ...
        if(Number(subarr1[0]) <= Number(subarr2[0]) && Number(subarr1[1]) >= Number(subarr2[1])){
            ...
            overlap += 1;
        } else if(Number(subarr1[1]) <= Number(subarr2[1]) && Number(subarr1[1]) >= Number(subarr2[0])) {
            overlap += 1;
        } else if(Number(subarr1[0]) >= Number(subarr2[0]) && Number(subarr1[0]) <= Number(subarr2[1])) {
            overlap += 1;
        }
    }
    console.log("overlap: ", overlap);
